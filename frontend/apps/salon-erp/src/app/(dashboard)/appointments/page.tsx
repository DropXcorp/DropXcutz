...
export default function AppointmentsPage() {
  ...
  const filteredAppointments = useMemo(
    () =>
      appointments.filter((appointment) => {
        ...
        const matchesDate =
          date === "all" ||
          (date === "today" && day === now.getTime()) ||
          (date === "tomorrow" && day === now.getTime() + 86400000) ||
          (date === "week" &&
            day >= now.getTime() &&
            day < now.getTime() + 7 * 86400000) ||
          (date === "month" &&
            new Date(appointment.schedule.date).getMonth() === now.getMonth());
        return (
          matchesSearch &&
          matchesEmployee &&
          matchesStatus &&
  ...
  const overview = useMemo(
    () => ({
      ...
      walkInPercentage: `${Math.round((appointments.filter((item) => item.appointment.source === "Walk-in").length / Math.max(appointments.length, 1)) * 100)}%`,
      onlineAppointments: appointments.filter(
        (item) => item.appointment.source === "Online",
      ).length,
      onlinePercentage: `${Math.round((appointments.filter((item) => item.appointment.source === "Online").length / Math.max(appointments.length, 1)) * 100)}%`,
  ...
  const saveAppointment = async (data: AppointmentFormData) => {
    ...
    if (selectedAppointment) {
      ...
      let customer =
        customers.find((item) => item.phone === data.customerPhone) ??
        customers.find(
          (item) => item.name.toLowerCase() === data.customerName.toLowerCase(),
        );
      if (!customer)
  ...
  const handleExport = (type: ExportType) => {
    ...
    const content = [
      "Appointment ID,Customer,Stylist,Date,Time,Amount,Status",
