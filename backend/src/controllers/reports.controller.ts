...
async function snapshot(range: Range) {
  ...
  const [
    ...
    prisma.invoice.aggregate({
      where: { ...invoiceWhere, status: "REFUNDED" },
    ...
    prisma.payrollRun.findMany({
      where: payrollWhere,
      select: { baseSalary: true, commission: true, deductions: true },
    }),
    prisma.platformInvoice.aggregate({
      where: { createdAt: { gte: range.from, lt: range.to }, status: "PAID" },
    ...
    prisma.salon.findMany({
      select: { id: true, salonName: true, code: true },
    }),
  ]);
  const payouts = payroll.reduce(
    (sum, item) =>
      sum +
      decimal(item.baseSalary) +
      decimal(item.commission) -
      decimal(item.deductions),
    0,
  );
  const revenue = decimal(paid._sum.totalAmount);
  const gst = decimal(paid._sum.taxAmount);
  const refunds = decimal(
    refunded._sum.amountPaid || refunded._sum.totalAmount,
  );
  ...
  for (const invoice of invoices) {
    ...
    if (invoice.status === "PAID") {
      row.revenue += decimal(invoice.totalAmount);
      salon.revenue += decimal(invoice.totalAmount);
    }
    if (invoice.status === "REFUNDED") {
      row.refunds += decimal(invoice.totalAmount);
      salon.refunds += decimal(invoice.totalAmount);
    }
    daily.set(day, row);
    salons.set(invoice.salonId, salon);
  }
  ...
  return {
    metrics: {
      ...
      gst,
      refunds,
      expenses: decimal(expenses._sum.amount),
      payouts,
      profitLoss: revenue - refunds - costs,
      paidInvoiceCount: paid._count,
      refundedInvoiceCount: refunded._count,
      platformBillingRevenue: decimal(platformPaid._sum.totalAmount),
      platformBillingGst: decimal(platformPaid._sum.taxAmount),
      platformBillingInvoiceCount: platformPaid._count,
    },
    daily: [...daily.entries()].map(([date, values]) => ({ date, ...values })),
    salons: [...salons.entries()]
      .map(([salonId, values]) => ({
        salonId,
        salonName: nameMap.get(salonId)?.salonName ?? "Deleted salon",
        code: nameMap.get(salonId)?.code ?? "—",
        ...values,
      }))
...
export async function financialReport(request: Request, response: Response) {
  ...
  const comparison =
    input.compareFrom || input.compareTo
      ? await snapshot(
          resolveRange({ from: input.compareFrom, to: input.compareTo }),
        )
      : null;
  response.json({
    data: {
      range: {
        ...
        to: new Date(currentRange.to.getTime() - 1),
      },
      current,
      comparison,
    },
...
export async function financialReportCsv(request: Request, response: Response) {
  ...
  response.setHeader(
    "content-disposition",
    `attachment; filename="dropxcutz-financial-${range.from.toISOString().slice(0, 10)}.csv"`,
  );
  response.send(csv);
}
