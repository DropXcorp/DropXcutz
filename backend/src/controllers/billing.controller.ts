...
const invoiceNumber = () =>
  `PLT-${new Date().getFullYear()}-${randomUUID().slice(0, 8).toUpperCase()}`;
...
export async function createPlatformInvoice(
  ...
  if (input.subscriptionId) {
    const subscription = await prisma.subscription.findFirst({
      where: { id: input.subscriptionId, salonId: salon.id },
    });
    if (!subscription)
      throw new ApiError(400, "Subscription does not belong to this salon.");
  }
  ...
  const paymentLink = await createRazorpayPaymentLink({
    ...
    customer: {
      name: salon.adminName,
      email: salon.adminEmail,
      contact: salon.phone,
    },
...
export async function handleRazorpayWebhook(
  ...
  const eventId =
    request.header("x-razorpay-event-id") ?? createRazorpayEventId(rawBody);
  try {
    await prisma.razorpayWebhookEvent.create({
      data: { eventId, eventType: event.event ?? "unknown", payload: event },
    });
  } catch (error) {
    if (
