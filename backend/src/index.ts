...
const origins = (process.env.FRONTEND_ORIGINS ?? "http://localhost:3000,http://localhost:3001").split(",").map((origin) => origin.trim());
...
const server = app.listen(port, () => console.info(`Salon API listening on http://localhost:${port}`));
